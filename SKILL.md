...
## Референсный API Sigma (Regulation Management) — НЕ зависимость

Прототип Sigma (`sigma.docs`) использовался как **референс контракта** для собственной модели RAGRAF. На production-инстансе RAGRAF к этому хосту не обращается: source of truth — локальный DuckDB-store, фикстуры — seed (по дефолту `USE_FIXTURES=true`, `WRITEBACK_UPSTREAM=false`). Поверхность ниже описана для совместимости — на случай если кто-то захочет включить round-trip через `regulation_client` (`/docs`, `/openapi.json` upstream — это **исходный документ-источник**, а не наша рабочая зависимость).

Реальная поверхность референса (FastAPI 0.1.0):
